...
- (UITableView *)baseSearchTableView
{
    if (!_baseSearchTableView) {
        UITableView *baseSearchTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - kDockStatusHeight) style:UITableViewStyleGrouped];
...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:self action:@selector(cancelDidClick)];
    ...
    [btn setTitleColor:kXSF_COLOR_158 forState:UIControlStateNormal];
    btn.titleLabel.textAlignment =  NSTextAlignmentRight;
    btn.titleLabel.font = [UIFont systemFontOfSize:14];
    btn.tag = 2;
    [btn addTarget:self action:@selector(cancelDidClick) forControlEvents:UIControlEventTouchUpInside];
    CGSize size = [self getTextSize:@"取消" fontSize:btn.titleLabel.font];
...
{
    [self.searchBar resignFirstResponder];
//    [XSFAlertView showAlertView:@"确定清空搜索历史吗?" leftBtnTitle:@"取消" rightBtnTitle:@"清除" leftBlock:^{
//        
//    } rightBlock:^{
